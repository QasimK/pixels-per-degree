...
function calculate (info) {
  /* Take in info = { // in metric form (cm)
    ...
    refreshRate and colorDepth are optional and used to calculate
    the data rates and bandwidth requirements

    return { // in metric form
      aspectRatio: 2.3755435,
      aspectRatioW: 21, // or "uncommon"
      aspectRatioH: 9,  // or "uncommon"
      scaledResolutionWidth: 2560,
      scaledResolutionHeight: 1080,
      horizontalFOV: ,
      PPD: ,
      scaledPPD: ,
  ...
  */
  ...
  const basics = calculateBasics(
    info.diagonalLength, info.resolutionWidth, info.resolutionHeight, info.distance, info.scaling)
  const dataRates = calculateDataRates(
    info.resolutionWidth, info.resolutionHeight, info.refreshRate, info.colorDepth
  )
...
function calculateBasics (
  ...
  // Scaled resolution
  var scaledResolutionWidth = resolutionWidth / scaling
  var scaledResolutionHeight = resolutionHeight / scaling
  ...
  var scaledPPD = scaledResolutionWidth / horizontalFOV
  ...
  return {
    ...
    aspectRatioW,
    aspectRatioH,
    scaledResolutionWidth,
    scaledResolutionHeight,
    horizontalFOV,
    PPD,
    scaledPPD,
